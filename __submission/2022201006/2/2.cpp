        ...
        for(int intr=0; intr<nvertices; intr++){
            ...
            if(my_rank == root_rank){
                for(int procno=root_rank+1; procno<nproc; procno++){
                    ...
                    intr_dest = adj[intr];
                    MPI_Isend(src_dest,  nrows_to_send*nvertices, MPI_INT, procno, procno, MPI_COMM_WORLD, req_sd);
                    MPI_Isend(intr_dest, nvertices, MPI_INT, procno, procno, MPI_COMM_WORLD, req_id);
            ...
            // , process as soon as receive complete, then send thr processed row back to root
                ...
                MPI_Irecv(src_dest, nrows_to_recv*nvertices, MPI_INT, root_rank, my_rank, MPI_COMM_WORLD, req_sd);
                MPI_Irecv(intr_dest, nvertices, MPI_INT, root_rank, my_rank, MPI_COMM_WORLD, req_id);


                // wait for receive, process the rows and send it back to root
                ...
                // log("r: "+ my_rank_s); print2d(adj, nvertices);
                log("r: "+ my_rank_s);
            ...
            if(my_rank == root_rank){
                ...
                for(int procno=root_rank+1; procno<nproc; procno++){
                    ...
                    req_sd   = new MPI_Request;
                    receives.push_back(req_sd);
                    MPI_Irecv(src_dest, nrows_to_recv*nvertices, MPI_INT, procno, procno, MPI_COMM_WORLD, req_sd);
...
// run: mpic++ mpifwcf.cpp -o mpifwcf && mpiexec -n 5 ./mpifwcf < fw.inp
// run: mpic++ mpifwcf.cpp -o mpifwcf && mpirun -n 12 --oversubscribe --use-hwthread-cpus ./mpifwcf < fw.inp
